...
bool SendFileControl::send_pfile(shared_ptr<boost::asio::ip::tcp::socket> socket_ptr, const FileType &filetype, const char *filepath, const string &id, const string &suffix)
{
    /* 检测大小 */
    unsigned int file_size = getFileSize(filepath);
    if (file_size == 0) {
        ...
        p.packetHeader.purpose = Purpose::GetFile;
        p.packetHeader.data_size = 0;
        boost::asio::write(*socket_ptr, boost::asio::buffer(&p, sizeof(NetPacketHeader)));
        return false;
    }
    ...
    int ret = 0;
    ret = boost::asio::write(*socket_ptr, boost::asio::buffer(p, sizeof(NetPacketHeader)+BUF_SIZE+file_size));
    safe_delete(p);  //释放内存
...
bool SendFileControl::send_vfile(shared_ptr<boost::asio::ip::tcp::socket> socket_ptr, const FileType &filetype, const char *filepath, const string &videoId, const string &publisherId, const string &suffix)
{
    /* 检测大小 */
    unsigned int file_size = getFileSize(filepath);
    if (file_size == 0) {
...
void SendFileControl::send_file(shared_ptr<boost::asio::ip::tcp::socket> socket_ptr, const string &buf)
{
    json jsonMsg = json::parse(buf);
    string dirpath, filepath, command;
    if (jsonMsg["filetype"] == FileType::ProfilePicture) {
        ...
        dirpath = PROFILE_PICTURE_URL;
        filepath = dirpath + string(jsonMsg["id"]) + pictureSuffix;
        if (send_pfile(socket_ptr, FileType::ProfilePicture, filepath.data(), string(jsonMsg["id"]), pictureSuffix)) {
            std::cout << "发送成功" << std::endl;
        } else std::cout << "发送失败" << std::endl;
    } else if (jsonMsg["filetype"] == FileType::Video) {
        /* 点播视频文件 */
